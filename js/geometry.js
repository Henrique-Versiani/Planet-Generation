...
class IcoSphere {
    ...
    generateColors(waterLevel, noiseFn, strength, freq, maxDepth, palette) {
        ...
        for (let i = 0; i < this.vertices.length; i += 3) {
            ...
            const nx = x / currentLen, ny = y / currentLen, nz = z / currentLen;
            let r, g, b;
            if (currentLen <= waterLevel + 0.001) {
                ...
                let depthFactor = Math.max(0.0, Math.min(1.0, depth / maxDepth));
    ...
    getTreeGeometry(treeSeed) {
        ...
        // OTIMIZAÇÃO: reutiliza esfera de folhagem em cache (evita new IcoSphere(0) repetido)
        if (!_cachedFoliageSphere) _cachedFoliageSphere = new IcoSphere(0);
        const foliage = _cachedFoliageSphere;
    ...
    buildTreesOnly(plantedPositions, waterLevel, noiseFn, strength, freq, currentTime) {
        const tV = [], tC = [];
        const growDuration = 1500;
        const mat = mat4.create(), q = quat.create(), up = vec3.fromValues(0, 1, 0);
        const pos = vec3.create(), norm = vec3.create(), treePos = vec3.create();
        ...
        for (let i = 0; i < plantedPositions.length; i++) {
            ...
            const height = 1.0 + (noiseVal * strength);
            ...
            if (p.startTime) {
                ...
                if (elapsed < growDuration) scale = Utils.easeOutElastic(elapsed / growDuration);
            }
            ...
            for (let j = 0; j < treeGeom.v.length; j += 3) {
                ...
                tV.push(treePos[0], treePos[1], treePos[2]);
                tC.push(treeGeom.c[j], treeGeom.c[j+1], treeGeom.c[j+2]);
            }
        }

        const tN = [];
        for (let i = 0; i < tV.length; i += 9) {
            const v1x = tV[i+3]-tV[i], v1y = tV[i+4]-tV[i+1], v1z = tV[i+5]-tV[i+2];
            const v2x = tV[i+6]-tV[i], v2y = tV[i+7]-tV[i+1], v2z = tV[i+8]-tV[i+2];
            let nx = v1y*v2z - v1z*v2y, ny = v1z*v2x - v1x*v2z, nz = v1x*v2y - v1y*v2x;
            const len = Math.sqrt(nx*nx + ny*ny + nz*nz) || 1;
            nx/=len; ny/=len; nz/=len;
            tN.push(nx,ny,nz, nx,ny,nz, nx,ny,nz);
        }
        return { vertices: tV, normals: tN, colors: tC };
    }
